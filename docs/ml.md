...
2. **Проход 2** — CV-оценка (Sharpe ratio) и финальный `fit` только на отобранных признаках.
...
Оптимизация порога уверенности per-ticker (после финального фита) тоже максимизирует Sortino.
...
«эта точка входа была бы прибыльна с реальными SL/TP» — ровно то, что модель должна предсказывать.

`ML_THRESHOLD` теперь задаёт минимальный **чистый** P&L (после комиссии и НДФЛ) для BUY/SELL метки.
