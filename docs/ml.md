...
## Метрика оптимизации (Sharpe ratio)

Вместо F1-macro Optuna HPO и CV-оценка ансамбля используют **Sharpe ratio P&L-симуляции**:

```
Sharpe = mean(P&L) / std(P&L)
```
...
- P&L сделки: `(exit - entry) / entry - 2 × commission_pct`
...
- Штрафует неустойчивую прибыль (высокое std) — Sharpe предпочтёт стабильный доход перед случайными всплесками
- Штрафует "молчаливые" модели: при < `ML_SHARPE_MIN_TRADES` сделок → возвращает 0.0

Оптимизация порога уверенности per-ticker (после финального фита) тоже максимизирует Sharpe.
...
**Ночное обучение включает CV** (n_splits=3) — метрика Sharpe обновляется каждую ночь.
...
Критерий — Sharpe ratio P&L-симуляции: `_sharpe_score(_simulate_pnl(...))`.
