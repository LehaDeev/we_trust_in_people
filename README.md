...
Автоматизированный торговый бот для Тинькофф Инвестиции на Python.
ML-ансамбль предсказывает ожидаемый P&L для 27 акций MOEX на часовых свечах,
бот открывает/закрывает позиции по сигналам, управляет рисками через биржевые SL/TP стоп-ордера.

**Торговая стратегия:** ML-driven swing trading (long-only, MOEX, 1h).
Модель: `RankEnsemble` = LightGBM + ExtraTrees + HistGBM с z-score нормализацией и адаптивными весами.
Position sizing по методу Fixed Fractional Risk, ATR-based динамические SL/TP, режимный фильтр тренда.
