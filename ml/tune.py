...
Оценка производится через WalkForwardSplit — роллинговую кросс-валидацию
с gap=lookahead и embargo=ML_WF_EMBARGO для устранения утечки меток.
Метрика: средняя корреляция Спирмена между pnl_pred и реальным P&L —
...
# ── Роллинговая кросс-валидация с gap и embargo ───────────────────────────────
