...
Группы признаков (62 признака):
    ...
    - Торговая сессия MOEX: первый час, последний час, таймер сессии [0→1]
...
FEATURE_COLUMNS: list[str] = [
    ...
    # ── Признаки торговой сессии MOEX ────────────────────────────────────────
    # Бинарные флаги выделяют особые периоды с иной динамикой объёмов и волатильности.
    # Деревья используют их как прямые split-пороги (0/1) — точнее, чем hour_sin/cos
    # для дискретных «особых» интервалов. minutes_to_close — линейный таймер сессии.
    "is_open_hour", "is_close_hour", "minutes_to_close",
    # ── USD/RUB курс ─────────────────────────────────────────────────────────
...
def compute_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ── Признаки торговой сессии MOEX ────────────────────────────────────────
    # Бинарные флаги точнее передают особые периоды сессии, чем hour_sin/cos:
    # деревья используют их как прямой сплит 0/1, не интерполируя по синусоиде.
    # Источник границ: регламент торгов MOEX (основная секция, акции).
    hour_int   = time_col.dt.hour.values.astype(np.int32)
    minute_int = time_col.dt.minute.values.astype(np.int32)

    # Первый час торгов (09:50 – 10:59 МСК включительно):
    # повышенная волатильность, переоценка overnight-позиций, gap-открытия,
    # приход informed traders — основной источник волатильности открытия на MOEX.
    # Граница 09:50 соответствует началу pre-trading auction; основная сессия с 10:00.
    df["is_open_hour"] = (
        ((hour_int == 9) & (minute_int >= 50)) | (hour_int == 10)
    ).astype(np.float64)

    # Последний час торгов (18:00 – 18:40 МСК включительно):
    # направленные движения на закрытие, высокий объём,
    # аукцион закрытия (18:40–18:50) — ликвидность концентрируется у цены дня.
    df["is_close_hour"] = (hour_int == 18).astype(np.float64)

    # Таймер сессии, нормированный к [0, 1]:
    #   1.0 = начало основной сессии (10:00 МСК)
    #   0.0 = конец сессии (18:40 МСК) и вне торговых часов.
    # Помогает модели учитывать «торопливость» участников к закрытию:
    # ликвидность и поведение цены систематически меняются на протяжении дня.
    _SESSION_START_MIN = 10 * 60        # 10:00 МСК = 600 мин от полуночи
    _SESSION_END_MIN   = 18 * 60 + 40   # 18:40 МСК = 1120 мин от полуночи
    _SESSION_TOTAL     = _SESSION_END_MIN - _SESSION_START_MIN  # 520 мин

    total_minutes = (hour_int * 60 + minute_int).astype(np.float64)
    in_session = (total_minutes >= _SESSION_START_MIN) & (total_minutes <= _SESSION_END_MIN)
    df["minutes_to_close"] = np.where(
        in_session,
        (_SESSION_END_MIN - total_minutes) / _SESSION_TOTAL,
        0.0,
    )
